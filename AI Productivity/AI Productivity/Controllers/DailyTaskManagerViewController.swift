...
class DailyTaskManagerViewController: UIViewController, UIScrollViewDelegate {
    ...
    // Floating add task button (no longer used since we use the navigation bar item).
    ...
    override func viewDidLoad() {
        ...
        setupAddTaskButton() // <-- Add this line
        
        // Remove the "+" button from navigation bar.
        // Previously: navigationItem.rightBarButtonItem = addButtonItem
...
extension DailyTaskManagerViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {  
        ...
        cell.doneButtonAction = { [weak self] in
            guard let self = self,
                  let idx = self.tasks.firstIndex(where: { $0.title == task.title && $0.dueDate == task.dueDate }) else { return }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        detailView.deleteAction = { [weak self, weak detailView] in
            ...
            if task.recurrence != "None" {
                ...
                if let idx = self.tasks.firstIndex(where: { $0.title == task.title && $0.dueDate == task.dueDate }) {
...
extension DailyTaskManagerViewController: NewTaskViewControllerDelegate {
    ...
    func didEditTask(_ task: ExtendedTask) {
        // Update the task in the master list.
        if let index = tasks.firstIndex(where: { $0.title == task.title && $0.dueDate == task.dueDate }) {
            tasks[index] = task
        }
        saveTasks()
        filterTasksForSelectedDate()
...
extension DailyTaskManagerViewController {
    func presentEditTaskView(for task: ExtendedTask) {
        ...
        editTaskVC.editingTask = task  // Now compiles because editingTask is ExtendedTask?
...
extension DailyTaskManagerViewController {
    ...
    private func deleteAllRecurringTasks(_ task: ExtendedTask) {
        // Remove all tasks in the series (for simplicity, matching by title and recurrence)
        tasks.removeAll(where: { $0.title == task.title && $0.recurrence == task.recurrence })
        saveTasks()
        filterTasksForSelectedDate()
    }
}
